              ...
              <div className="w-full md:w-2/3 space-y-6">
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside text-gray-600 space-y-1">
                      {member.courses.map((course, index) => (
                        <li key={index}><Text as="span">{typeof course === 'string' ? course : (course as any).name || 'Course'}</Text></li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {(member as any).description && (
                  <div>
                    ...
                    <Text className="whitespace-pre-line">{(member as any).description}</Text>
                  </div>
                )}
              </div>
            </div>
